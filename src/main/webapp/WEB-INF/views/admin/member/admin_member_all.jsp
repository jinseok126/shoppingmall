		...
		<table class="table table-sm table-striped table-hover">
			...
			<tbody>
				...
				<c:if test="${empty map.searchOption}">
					...
					<tr align="center">
					  <td>
					 	<input type="hidden" id="id_${st.count}" value="${row.id}">
						<a href="${path}/sm/admin/view.do?id=${row.id}">${row.id}</a>
					  </td>
					  <td>${row.pw}</td>
					  <td>${row.name}</td>
					  <td>${row.gender}</td>
					  ...
					  <td><fmt:formatDate value="${row.birthday}" pattern="yyyy년 M월 d일" /></td>
					  <td>${row.joindate}</td>
							  ...
							  <c:if test="${row.role != 'role_admin'}">
								<select id="role_change">
								  <option value="role_manager"
									<c:out value="${map.searchOption=='role_manager'?
																'selected':''}"/>>매니저</option>
								  <option value="role_user"
									<c:out value="${map.searchOption=='role_user'?
																'selected':''}"/>>사용자</option>
								</select> &nbsp;
			...
			<c:if test="${empty map.searchOption && count > 5}">
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						...
						<li class="page-item">
						<a class="page-link"
							href="${path}/sm/admin/allMember.do?start=${rownum-10}
													&end=${rownum-1}">Previous</a></li>
						<li class="page-item">
						<a class="page-link"
							href="${path}/sm/admin/allMember.do?start=${rownum+10}
													&end=${rownum+19}">Next</a></li>
					</ul>
				</nav>
			</c:if>
			...
			<c:if test="${not empty map.searchOption && map.count > 5}">
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						...
						<li class="page-item">
							<a class="page-link" href="${path}/sm/admin/search/list.do?
															start=${map.start-10}
															&end=${map.start-1}
															&searchOption=${map.searchOption}
															&keyword=${map.keyword}">Previous</a>
						</li>
						<li class="page-item">
							<a class="page-link" href="${path}/sm/admin/search/list.do?
														start=${map.start+10}
...
<div class="row mb-2 col-lg-12">
	<form method="get"
		  action="${path}/sm/admin/search/list.do?
								   ...
								   searchOption=${map.searchOption}&
								   keyword=${map.keyword}"
		  class="mx-auto" style="font-size:10px">
		
		<div class="form-inline" >
		
			<!-- searchOption을 선택하기위한 select 태그 -->
			<select name="searchOption" class="form-control col-lg-3" style="font-size:10px">
				<option value="all"
			...
			<input type="text" name="keyword" id="keyword" value="${map.keyword}" 
				   class="form-control col-lg-7" style="font-size:10px">
			<input type="submit" value="검색" class="form-control col-lg-2" style="font-size:10px">
